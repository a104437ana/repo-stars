...
export function renderStarsSVG({ repo, count, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, animate = true }) {
  ...
  const polygons = stars
    .map((s) => {
      ...
      return `  <polygon points="${pts}" fill="#ffd54a" opacity="${s.opacity.toFixed(2)}" stroke="#7a4e12" stroke-width="1.2" stroke-opacity="0.55" paint-order="stroke fill" filter="url(#glow)"${cls}${style}/>`;
    })
    .join("\n");
