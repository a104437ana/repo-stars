 ...
 */
export function renderStarsSVG({ count, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, animate = true }) {
  ...
  if (useFixedLayout) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      stars.push({
        points: starPoints(x, y, radius, rot),
        opacity,
        delay: Math.round(rnd() * 4000),
        duration: Math.round(2600 + rnd() * 2200),
      });
  ...
  const polygons = stars
    .map((s) => {
      ...
      return `  <polygon points="${s.points}" fill="#ffd54a" opacity="${s.opacity.toFixed(2)}" stroke="#7a4e12" stroke-width="1.2" stroke-opacity="0.55" paint-order="stroke fill" filter="url(#glow)"${cls}${style}/>`;
    })
    .join("\n");
